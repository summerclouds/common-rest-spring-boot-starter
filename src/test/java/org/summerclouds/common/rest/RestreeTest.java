...
//}
//
//for (Map.Entry<String, ? extends ServletRegistration> reg : servletContext.getServletRegistrations().entrySet()) {
//	System.out.println("Reg: " + reg.getKey() + " " + reg.getValue().getMappings() + " " + reg.getValue().getClassName());
//}
	...
	public void testPublic() throws Exception {
		MvcResult ret = this.mockMvc.perform(get("/rest/public")).andDo(print()).andExpect(status().isOk()).andReturn();
	...
	public void testPublicSleep() throws Exception {
		...
		MvcResult ret = this.mockMvc.perform(post("/rest/public").param("_action", "sleep")).andDo(print()).andExpect(status().isOk()).andReturn();
		watch.stop();
		String content = ret.getResponse().getContentAsString();
		assertTrue(content.contains("\"_timestamp\""));
	...
	public void testNotFound() throws Exception {
		MvcResult ret = this.mockMvc.perform(get("/rest/notexists")).andDo(print()).andExpect(status().isNotFound()).andReturn();
		String content = ret.getResponse().getContentAsString();
