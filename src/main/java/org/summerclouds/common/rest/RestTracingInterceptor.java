...
//	@Value("${org.summerclouds.common.rest.tracePathParts:false}")
	private CfgBoolean tracePathParts = new CfgBoolean("org.summerclouds.common.rest.tracePathParts", false);
//	@Value("${org.summerclouds.common.rest.traceParameters:false}")
	private CfgBoolean traceParameters = new CfgBoolean("org.summerclouds.common.rest.traceParameters", false);
//	@Value("${org.summerclouds.common.rest.traceHeaders:false}")
	private CfgBoolean traceHeaders = new CfgBoolean("org.summerclouds.common.rest.traceHeaders", false);
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (MString.isSet(traceId))
			response.setHeader("X-Summer-TraceId", traceId );
		String subject = MString.valueOf(MSecurity.getCurrent());
		if (MString.isSet(subject))
			response.setHeader("X-Summer-Subject", subject );
		...
		if (tracePathParts.value()) {
			...
			for (int i = 1; i < parts.length; i++)
				span.setTag("request.part" + i, parts[i]);
		}
		...
		if (traceHeaders.value()) {
			...
			while (enu.hasMoreElements()) {
				...
				if (sb != null)
					span.setTag("request.header_" + name, sb.toString());
			}
		}
		return true;
	}
